...
}).refine((v) => !v.latitude || CoordinateSchema.safeParse({ latitude: v.latitude, longitude: v.longitude }).success, {
...
}).strict().refine((v) => CoordinateSchema.safeParse(v).success, { message: 'coordinate is outside supported bounds' });
